...
train_info_path = r"E:\myworkspace\depression\depression\hxq_model\daic_woz_dataset\train_split_Depression_AVEC2017.csv"
train_info_data = pd.read_csv(train_info_path)

tran_data = []
hdr_path = r"E:\myworkspace\depression\depression\hxq_model\daic_woz_dataset\hdr"
for file in os.listdir(hdr_path):
    ID = file.split("_")[0]
    label = 0
    for i in range(train_info_data.shape[0]):
        p_id = train_info_data.iloc[i, 0]
        if int(p_id) == int(ID):
            label = train_info_data.iloc[i, 2]
            break
...
tcn_plot = "qiao_vidio_1.png"
plot_model(tcn, to_file=tcn_plot)
